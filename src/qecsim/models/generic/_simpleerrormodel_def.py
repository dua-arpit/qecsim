...
class SimpleErrorModel_def(ErrorModel):
    """
...
class DepolarizingErrorModel_def(SimpleErrorModel_def):
    """
...
class BitFlipErrorModel_def(SimpleErrorModel_def):
    """
    ...
    * 0: Z
    ...
    @functools.lru_cache()
    def probability_distribution(self, probability):
        """See :meth:`qecsim.model.ErrorModel_def.probability_distribution`"""
        p_x = probability
        p_y = p_z = 0
        p_i = 1 - sum((p_x, p_y, p_z))
        return p_i, p_x, p_y, p_z
    ...
    @property
    def label(self):
        """See :meth:`qecsim.model.ErrorModel_def.label`"""
        return 'Bit-flip'


@cli_description('Pr I,X,Y,Z is 1-p,0,0,p')
class PhaseFlipErrorModel_def(SimpleErrorModel_def):
    """
...
class BitPhaseFlipErrorModel_def(SimpleErrorModel_def):
    """
