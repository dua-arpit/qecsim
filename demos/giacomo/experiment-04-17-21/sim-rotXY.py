...
def parallel_step_code(code,error_model,decoder,max_runs,perm_rates,code_name,layout,error_probabilities,realization_index):
    ...
    for error_probability_index,error_probability in enumerate(error_probabilities):
        # perm_mat,perm_vec= deform_matsvecs(code,decoder,error_model)
        [[pL_list[error_probability_index],std_list[error_probability_index]],[pL_list[error_probability_index],std_list[error_probability_index]]] = app_defp.run_defp(code,error_model,decoder,error_probability,perm_rates,code_name,layout,max_runs)

    return [[pL_list,std_list] ,[oL_samples_list, std_samples_list]]
