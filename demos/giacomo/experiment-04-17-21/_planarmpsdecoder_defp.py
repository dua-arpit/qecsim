...
class PlanarMPSDecoder_defp(Decoder):
    r"""
    ...
    * The code is optimised to evaluate cosets that differ only in the last column/row together. It is important,
    ...
    def decode(self, code, perm_mat, syndrome,
        ...
        #print(coset_ps,"  ", max_coset_p,"   ",sum(coset_ps)) 
        # logging
        ...
        return max_coset_p, max_recovery.to_bsf()

    @property
    def label(self):
        """See :meth:`qecsim.model.Decoder.label`"""
        params = [('chi', self._chi), ('mode', self._mode), ('stp', self._stp), ('tol', self._tol), ]
        return 'Planar MPS_defp ({})'.format(', '.join('{}={}'.format(k, v) for k, v in params if v))
